...
public final class StatefulDecoder extends Decoder {
    ...
    private void newFormat(int format) {
        ...
        if (format == R.id.idFormatUpperCase || format == R.id.idFormatLowerCase || format == R.id.idFormatSentence)
            this.format = format;
        else
    ...
    public String decode(int x) {
        ...
        if (s != null) {
            if(format == R.id.idFormatLowerCase)
                    return s.toLowerCase();
                else if(format == R.id.idFormatSentence) {
                if (s.length() >= 1) {
